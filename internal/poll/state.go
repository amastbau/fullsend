...
func (p *Poller) loadPollState(ctx context.Context, owner, repo string) (persistedPollState, error) {
	...
	// Fail closed when no secret is configured: poll state lives in a
	// Developer-writable package file, so an unsigned document cannot be
	// trusted. FULLSEND_DISPATCH_SECRET is auto-provisioned by repos
	// install/converge and re-signs any pre-#7317 unsigned state during
	// that step, so a missing secret here means a broken/incomplete
	// install to repair rather than forgeable state to silently trust.
...
// EnsureGitLabPollStateSigned re-signs an existing but unsigned poll
// state package document in place using dispatchSecret. It is a no-op
// (returns false, nil) when dispatchSecret is empty, when no poll state
// package exists yet (a fresh install has nothing to sign — the first
// poll writes a signed document), or when the existing document already
// carries a signature.
//
...
// FULLSEND_DISPATCH_SECRET, the runtime poller fails closed on the
// unsigned document. Re-signing it here — while the trusted operator's
// Maintainer-level client is running install/converge — preserves the
// existing watermarks/label state/dispatch history (avoiding a 1-hour
// replay and re-dispatch storm) instead of discarding it. Corrupt
// (unparseable) documents are left untouched for the runtime
// fail-closed path to surface.
func EnsureGitLabPollStateSigned(ctx context.Context, client forge.Client, owner, repo, dispatchSecret string) (bool, error) {
	if dispatchSecret == "" {
		return false, nil
	}
	...
	state.HMAC = sig
	signed, err := json.Marshal(state)
	if err != nil {
		return false, err
	}
	if err := client.UploadPackageFile(ctx, owner, repo, pollStatePackageName, pollStatePackageVersion, pollStateFileName, signed); err != nil {
		return false, fmt.Errorf("re-signing poll state: %w", err)
	}
	return true, nil
}
