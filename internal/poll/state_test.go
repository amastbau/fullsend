...
// --- EnsureGitLabPollStateSigned tests ---
//
...
// FULLSEND_DISPATCH_SECRET, EnsureGitLabPollStateSigned re-signs the
// existing document in place so the fail-closed runtime poller accepts
// it instead of replaying an hour of history.

func TestEnsureGitLabPollStateSigned_ReSignsUnsigned(t *testing.T) {
	fc := forge.NewFakeClient()
	watermark := time.Date(2025, 3, 1, 9, 0, 0, 0, time.UTC).Format(time.RFC3339)
	...
	changed, err := EnsureGitLabPollStateSigned(context.Background(), fc, "testgroup", "testrepo", "test-secret")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !changed {
		t.Fatal("expected an unsigned document to be re-signed")
	}

	data, err := fc.DownloadPackageFile(context.Background(), "testgroup", "testrepo", pollStatePackageName, pollStatePackageVersion, pollStateFileName)
	if err != nil {
		t.Fatalf("download re-signed state: %v", err)
	}
	var state persistedPollState
	if err := json.Unmarshal(data, &state); err != nil {
		t.Fatalf("unmarshal re-signed state: %v", err)
	}
	if state.LastPollAtFull != watermark {
		t.Errorf("LastPollAtFull = %q, want %q (existing state must be preserved)", state.LastPollAtFull, watermark)
	}
	want, err := computeStateHMAC("test-secret", state)
	if err != nil {
		t.Fatalf("computeStateHMAC: %v", err)
	...
	if state.HMAC != want {
		t.Error("re-signed HMAC does not match the signature the runtime poller would verify")
	}
}

func TestEnsureGitLabPollStateSigned_NoopWhenAlreadySigned(t *testing.T) {
	fc := forge.NewFakeClient()
...
func TestEnsureGitLabPollStateSigned_NoopWhenNoPackage(t *testing.T) {
	fc := forge.NewFakeClient()
...
func TestEnsureGitLabPollStateSigned_NoopWhenSecretEmpty(t *testing.T) {
	fc := forge.NewFakeClient()
	...
	changed, err := EnsureGitLabPollStateSigned(context.Background(), fc, "testgroup", "testrepo", "")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if changed {
	...
	// The unsigned document must be left untouched for the runtime
	// fail-closed path (nothing to sign it with here).
	data, err := fc.DownloadPackageFile(context.Background(), "testgroup", "testrepo", pollStatePackageName, pollStatePackageVersion, pollStateFileName)
	if err != nil {
	...
	if state.HMAC != "" {
		t.Error("expected the document to remain unsigned when no secret is configured")
	}
}

func TestEnsureGitLabPollStateSigned_DownloadErrorPropagates(t *testing.T) {
	fc := forge.NewFakeClient()
	fc.Errors["DownloadPackageFile"] = fmt.Errorf("boom")

	_, err := EnsureGitLabPollStateSigned(context.Background(), fc, "testgroup", "testrepo", "test-secret")
	if err == nil {
		t.Fatal("expected a non-NotFound download error to propagate")
	}
}

func TestEnsureGitLabPollStateSigned_UploadErrorPropagates(t *testing.T) {
	fc := forge.NewFakeClient()
	if err := fc.UploadPackageFile(context.Background(), "testgroup", "testrepo", pollStatePackageName, pollStatePackageVersion, pollStateFileName, []byte(`{"last_poll_at_full":"2025-03-01T09:00:00Z"}`)); err != nil {
		t.Fatalf("seed unsigned state: %v", err)
	}
...
func TestEnsureGitLabPollStateSigned_LeavesCorruptDocument(t *testing.T) {
	fc := forge.NewFakeClient()
