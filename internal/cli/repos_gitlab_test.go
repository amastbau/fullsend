...
func TestProvisionGitLabDispatchSecret_ReSignsExistingUnsignedState(t *testing.T) {
	ctx := context.Background()
	fake := forge.NewFakeClient()
	...
	assert.Equal(t, "2026-01-01T00:00:00Z", state.LastPollAtFull, "existing watermark must be preserved")
	assert.NotEmpty(t, state.HMAC, "existing unsigned state should be re-signed in place")
	assert.Contains(t, buf.String(), "Signed existing poll state document")
