...
func TestEffectiveSetupAccessors(t *testing.T) {
	...
	assert.Equal(t, "us-west2", effectiveSetupRegion(githubSetupConfig{inferenceRegion: "us-west2"}, nil))
	assert.Equal(t, config.DefaultPerRepoInferenceRegion, effectiveSetupRegion(githubSetupConfig{}, nil))
	...
	assert.Equal(t, "europe-west1", effectiveSetupRegion(githubSetupConfig{}, cfg))
