...
| `validating files: policy: stat .../policies/base.yaml: no such file or directory` | The harness points at a policy file that is not committed next to it. Neither `fullsend github setup` nor CI creates one | Commit a policy file next to the harness, or set `policy:` to the URL of the fleet copy in [fullsend-ai/agents](https://github.com/fullsend-ai/agents). Re-running `agent new` on this agent name does not help — it collides on the harness that already exists (or, with `--force`, rewrites it) rather than just adding the missing policy |
