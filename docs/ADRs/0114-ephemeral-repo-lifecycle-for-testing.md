...
Fullsend's tests (behaviour tests today, admin e2e tests migrating later) need
...
2. **Pool of orgs with exclusive locking** ([ADR 0040](0040-org-pool-for-parallel-e2e-tests.md))
   — admin e2e tests acquired exclusive access to one of several
   pre-provisioned orgs via an atomic lock repo. This prevented concurrent-run
   collisions but serialized runs within each org and required pre-provisioned
   infrastructure per org.
...
- **Forge-specific teardown (retain vs. delete).** Creation is shared; teardown
  ...
  - **GitHub — retain and batch-prune.** Repos are retained after test
    ...
    the org's `bt-` repo count exceeds a threshold (currently 200), the
...
- **Forge-agnostic mechanism, forge-specific policy.** Creation and per-repo
  ...
  - GitHub's retain-and-prune path needs `AllRepoLister.ListAllOrgRepos` and
    ...
    `IsRateLimitError`); these are implemented for GitHub.
  - GitLab's delete-per-scenario path needs **only** `CreateRepo` / `DeleteRepo`
    (already implemented). It deliberately does **not** require
...
- **Behaviour test org.** Behaviour tests run against a single org, named
  ...
  `BEHAVIOUR_ORG`). The install/lifecycle client is currently GitHub; the
  install path still hardcodes `--forge github` and must become
  forge-selectable for GitLab. Pool coordination across multiple orgs is covered
  in [ADR 0115](0115-slot-based-pool-coordination-for-testing-orgs.md).

- **Build optimization.** `repos install` cross-compiles a Linux `fullsend`
  binary per install by default (via `--vendor` / `--fullsend-ref`). A
  `--fullsend-binary` flag exists to accept one pre-built binary and reuse it
  across installs, avoiding repeated cross-compilation. This optimization is
  **not yet wired into the behaviour framework** (the helper
  `CrossCompileCLIBinary` exists but is currently unused); adopting it is
  follow-up work to recover per-repo overhead introduced by ephemeral creation.
...
- **Concurrency is no longer capped by a name pool.** With a fixed pool, the
  13th concurrent scenario blocks on a free slot; with unique repos, concurrency
  is bounded only by the rate limit, so it scales with headroom instead of a
  hardcoded pool size.
